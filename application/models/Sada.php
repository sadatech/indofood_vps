...
class Sada extends CI_Model{
  ...
  public function getTopSku($filter)
  {
    ...
    $volume = $this->db->select(['id_produk ', 'SUM(qty) monthVolume','(select sd_kat.price from sada_produk p inner join sada_kategori sd_kat on p.id_kategori = sd_kat.id where p.id_produk = sada_produk_terjual.id_produk) price'])
...
public function contactTotal($arr = array(),$limit_excel)
{
  $q = "SELECT DISTINCT
    ...
    nama AS nama_store,
    (select COUNT(distinct id) from sada_form_contact sub_contact where sada_form_contact.user_id = sub_contact.user_id) contact_count,
    (select count(id) from sada_form_contact sub_contact where sub_contact.tipe = 'switching' and sada_form_contact.user_id = sub_contact.user_id) switching,
    (select count(id) from sada_form_contact sub_contact where sub_contact.tipe = 'newRecruit' and sada_form_contact.user_id = sub_contact.user_id) newRecruit,
